...
| `PSYWAVE` | `$95` | `DRAINING_KISS` | Fairy | Special | Confusing variable-damage move with poor player clarity. Replacing it creates a simple Fairy sustain option using existing drain behavior. | Appeared in one vanilla level-up entry and two egg move entries. Those references now use `CONFUSE_RAY` or `PSYBEAM` to avoid accidental Fairy coverage. | Implemented |
